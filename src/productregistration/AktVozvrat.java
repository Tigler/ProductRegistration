 ...
 * @author tigler
 */
public class AktVozvrat extends javax.swing.JFrame implements UpdatesDataInForms{

    @Override
    public void addDataInTable() {
        ResultSet resSet = null;
        try {
            resSet = ProductRegistration.st.executeQuery("select pk_aktvozvrat,aktvozvrat.num,postav.shortname,"
                    + " TO_CHAR(curdate, 'DD.MM.YYYY'),TO_CHAR(oformlen, 'DD.MM.YYYY')"
                    + " from aktvozvrat "
        ...
        try {
            for (int i = 0; i < jTable1.getRowCount(); i++) {
                resSet = ProductRegistration.st.executeQuery("select cost.zakupka,cost.nds,cost.nacenka,strvozvrat.count"
                        + " from strvozvrat "
                        ...
                        + " where strvozvrat.pk_actvozvrat=" + jTable1.getValueAt(i, 0)
                );
                double summa = 0;
                while (resSet.next()) {
    ...
    public AktVozvrat() {
        ...
        jButtonAdd.setToolTipText("Добавление акта возврата");
        jButtonEdit.setToolTipText("Изменение акта возврата");
        jButtonDelete.setToolTipText("Удаление акта возврата");
        try {
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Поставщик", "Дата", "Сумма", "Оформлено"
            }
        ));
        ...
        jLabel1.setText("Содержание акта возврата");

        jLabel2.setText("Акты возврата");
    ...
    private void jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAddActionPerformed
        ...
        AktVozvratEdit actVozvratEdit = new AktVozvratEdit(-1, 0);
        actVozvratEdit.setListenerCloseForm(new ListenerCloseForm(this));
        actVozvratEdit.setVisible(true);
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
        ...
        if (jTable1.getSelectedRow() != -1) {
            ...
            AktVozvratEdit actVozvratEdit = new AktVozvratEdit(pk, 1);
            actVozvratEdit.setListenerCloseForm(new ListenerCloseForm(this));
            actVozvratEdit.setVisible(true);
        } else {
         ...
         if (jTable1.getSelectedRow() != -1) {
            ...
            try {
                ...
                int result = JOptionPane.showOptionDialog(
                        null,
                        "Вы уверены, что хотите удалить акт возврата?",
                        "Удаление записи",
                ...
                if (result == JOptionPane.YES_OPTION) {
                    resSet = ProductRegistration.st.executeQuery("delete aktvozvrat where pk_aktvozvrat=" + jTable1.getValueAt(jTable1.getSelectedRow(), 0)
                    );
                    addDataInTable();
                }
            } catch (Exception ex) {
                System.out.println(ex);
                JOptionPane.showMessageDialog(rootPane, "Удаление акта возврата не возможно", "Удаление записи", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
        ...
        // TODO add your handling code here:
         try {
            ResultSet resSet = ProductRegistration.st.executeQuery("select tovar.pk_tovar,"
                    ...
                    + " inner join tovar on tovar.PK_tovar=strvozvrat.PK_tovar"
                    + " inner join cost on cost.PK_cost=tovar.PK_cost"
                    + " where strvozvrat.pk_actvozvrat=" + jTable1.getValueAt(jTable1.getSelectedRow(), 0)
            );
